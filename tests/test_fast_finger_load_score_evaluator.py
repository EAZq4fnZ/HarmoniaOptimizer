...
def test_evaluate_prepared_position_indexed_complete_delta_matches_complete(
) -> None:
    layout = make_layout()
    budgets = make_budgets()
    ...
    # Swap A and B so both letters are represented as changed.
    ...
    assert delta_result == pytest.approx(
        normal_result
    )


def test_evaluate_prepared_position_indexed_complete_delta_handles_empty_statistics(
) -> None:
    layout = make_layout()
